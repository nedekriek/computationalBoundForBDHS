...
def clause_generation(heuristic_function, bound: str, bound_type: str, locality: str, problem: str, bound_constraints_path_prefix: str, path_suffix: str, search_path: str):
    '''
    Generates all clauses for a search problem given a bound, its' type and locality eg. (lb, front_to_front, global).
    ...
    available_variable=max_node_id+1    #every method assumes that the available variable passed to it has not been used in any other clause and is not a node id
    ...
    must_expand_pairs, might_expand_pairs, not_must_expand_pair_clause = expand_clauses(dynamic_node_lower_bound_func, heuristic_function, must_expand_paired_buckets, might_expand_paired_buckets, buckets_f, buckets_b, bound_type,  c_star, epsilon_global, iota_global, global_info, available_variable)
    serialize(must_expand_pairs, bound_constraints_path_prefix+'must_expand_clauses'+path_suffix)
    must_expand_pairs = []
    serialize(might_expand_pairs, bound_constraints_path_prefix+'might_expand_clauses'+path_suffix)
    might_expand_pairs = []
    ...
    # bucket_aliases_f, bucket_aliases_b, available_variable, bucket = buckets_clauses(might_expand_paired_buckets, must_expand_paired_buckets, buckets_f, buckets_b, available_variable)
    # serialize(bucket, bound_constraints_path_prefix+'buckets_clauses'+path_suffix)
    # bucket = []

    # available_variable, gLimit = g_limit_clauses(bucket_aliases_f, bucket_aliases_b,  c_star, epsilon_global, available_variable)
    ...
    available_variable , at_least_one_collision = at_least_one_collision_clauses(common_child_collisions, nodes_adjacent_to_terminal, available_variable)
    ...
    return available_variable, collision_below_c_star
    # return None, None
