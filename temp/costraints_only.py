...
class Constraints:
    ...
    # def make_buckets(self, closed_list, epsilon_global, global_iota, global_information):
    #     buckets=dict()
    #     if global_information:
    #         for node in closed_list.values():
    #             key=(node.g, node.f, node.d, node.b, epsilon_global, global_iota)
    #             if key in buckets:
    ...
    #     else:
    #         for node in closed_list.values():
    #             key=(node.g, node.f, node.d, node.b, node.epsilon if node.epsilon else epsilon_global, node.iota if node.iota else global_iota)
    #             if key in buckets:
    #                 buckets[key].add(node)
    #             else:
    #                 buckets[key]=set([node])
    ...
    def run(self,  experiments=['experiment1','experiment2','experiment3','experiment4'], global_information = True): 
        ...
        for case in self.index:
            ...
            max_node_id, epsilon_global, global_iota, solutionNodesForward, solutionNodesBackward, closedListForward, closedListBackward = deserialize(self.searchDir+case+".obj")
            c_star=solutionNodesForward[0].g
            dummy_variable=max_node_id+1
            collision_below_c_star=False
            
            buckets_forward=self.make_buckets(closedListForward, epsilon_global, global_iota, global_information)
            buckets_backward=self.make_buckets(closedListBackward, epsilon_global, global_iota, global_information)
            ...
            if "experiment4" in experiments and self.solutionMustBeFoundBelowCStarLevel(solutionNodesForward, closedListBackward, c_star, epsilon_global, global_iota, global_information):
            ...
            data = [max_node_id, dummy_variable, c_star, collision_below_c_star]
